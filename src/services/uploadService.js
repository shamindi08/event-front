...
export const uploadSingleImageAlt = async (imageFile) => {
    ...
    try {
        ...
        console.log('Alternative upload response:', response);
        return {
            success: true,
            data: response.data,
        ...
        console.error('Alternative upload service error:', error);
        throw error;
    }
};
...
export const uploadSingleImageMultiAttempt = async (imageFile) => {
    ...
    for (const endpoint of endpoints) {
        for (const fieldName of fieldNames) {
            try {
                console.log(`Trying endpoint: ${endpoint} with field: ${fieldName}`);
                ...
                console.log(`Success with ${endpoint}/${fieldName}:`, response.data);
                ...
                console.log(`Failed ${endpoint}/${fieldName}:`, error.response?.status || error.message);
    ...
    try {
        ...
        for (const endpoint of endpoints) {
            try {
                ...
                const response = await api.post(endpoint, formData);
                console.log(`✅ Endpoint ${endpoint} is working:`, response.data);

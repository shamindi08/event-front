'use client';
import { useState } from 'react';
import { MdClose, MdStar, MdStarBorder, MdSentimentVerySatisfied, MdSentimentNeutral, MdSentimentVeryDissatisfied } from 'react-icons/md';

export default function FeedbackModal({ isOpen, onClose, event, onSubmit, existingFeedback = null }) {
  const [feedback, setFeedback] = useState({
    comment: existingFeedback?.comment || '',
    type: existingFeedback?.type || '',
    rating: existingFeedback?.rating || 0,
    isAnonymous: existingFeedback?.isAnonymous || false
  });
  const [hoveredRating, setHoveredRating] = useState(0);
  const [isSubmitting, setIsSubmitting] = useState(false);

  const feedbackTypes = [
    {
      type: 'positive',
      label: 'Positive',
      icon: MdSentimentVerySatisfied,
      color: 'text-green-500',
      bgColor: 'bg-green-100',
      borderColor: 'border-green-500'
    },
    {
      type: 'neutral',
      label: 'Neutral',
      icon: MdSentimentNeutral,
      color: 'text-yellow-500',
      bgColor: 'bg-yellow-100',
      borderColor: 'border-yellow-500'
    },
    {
      type: 'negative',
      label: 'Negative',
      icon: MdSentimentVeryDissatisfied,
      color: 'text-red-500',
      bgColor: 'bg-red-100',
      borderColor: 'border-red-500'
    }
  ];

  const handleSubmit = async (e) => {
    e.preventDefault();
    
    if (!feedback.comment.trim()) {
      alert('Please provide a comment');
      return;
    }

    if (!feedback.type) {
      alert('Please select feedback type');
      return;
    }

    if (feedback.rating === 0) {
      alert('Please provide a rating');
      return;
    }

    setIsSubmitting(true);
    
    try {
      await onSubmit({
        comment: feedback.comment.trim(),
        type: feedback.type,
        rating: feedback.rating,
        isAnonymous: feedback.isAnonymous,
        eventId: event._id
      });
      
      // Reset form if creating new feedback
      if (!existingFeedback) {
        setFeedback({
          comment: '',
          type: '',
          rating: 0,
          isAnonymous: false
        });
      }
      
      onClose();
    } catch (error) {
      console.error('Error submitting feedback:', error);
      if (error.response?.status === 400 && error.response?.data?.message?.includes('duplicate')) {
        alert('You have already submitted feedback for this event. You can only submit one feedback per event.');
      } else {
        alert('Failed to submit feedback. Please try again.');
      }
    } finally {
      setIsSubmitting(false);
    }
  };

  const handleTypeSelect = (type) => {
    setFeedback(prev => ({ ...prev, type }));
  };

  const handleRatingClick = (rating) => {
    setFeedback(prev => ({ ...prev, rating }));
  };

  if (!isOpen) return null;

  return (
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
      <div className="bg-white rounded-xl max-w-md w-full max-h-[90vh] overflow-y-auto">
        <div className="p-6">
          {/* Header */}
          <div className="flex items-center justify-between mb-4">
            <h3 className="text-xl font-semibold text-[#555879]">
              {existingFeedback ? 'Edit Feedback' : 'Leave Feedback'}
            </h3>
            <button
              onClick={onClose}
              className="text-gray-400 hover:text-gray-600 text-2xl"
            >
              <MdClose />
            </button>
          </div>

          {/* Event Info */}
          <div className="mb-6 p-3 bg-gray-50 rounded-lg">
            <h4 className="font-medium text-gray-800">{event.title}</h4>
            <p className="text-sm text-gray-600">
              {new Date(event.date).toLocaleDateString()} at {event.time}
            </p>
            <p className="text-sm text-gray-600">{event.location}</p>
            <p className="text-xs text-gray-500 mt-1">Organized by: {event.organizer}</p>
          </div>

          {/* Feedback Form */}
          <form onSubmit={handleSubmit}>
            {/* Rating */}
            <div className="mb-4">
              <label className="block text-sm font-medium text-gray-700 mb-2">
                Overall Rating *
              </label>
              <div className="flex items-center space-x-1">
                {[1, 2, 3, 4, 5].map((star) => (
                  <button
                    key={star}
                    type="button"
                    onClick={() => handleRatingClick(star)}
                    onMouseEnter={() => setHoveredRating(star)}
                    onMouseLeave={() => setHoveredRating(0)}
                    className="text-2xl text-yellow-400 hover:text-yellow-500 transition-colors"
                  >
                    {star <= (hoveredRating || feedback.rating) ? (
                      <MdStar />
                    ) : (
                      <MdStarBorder />
                    )}
                  </button>
                ))}
                <span className="ml-2 text-sm text-gray-600">
                  {feedback.rating > 0 && `${feedback.rating}/5`}
                </span>
              </div>
            </div>

            {/* Feedback Type Selection */}
            <div className="mb-4">
              <label className="block text-sm font-medium text-gray-700 mb-3">
                How was your experience? *
              </label>
              <div className="grid grid-cols-3 gap-2">
                {feedbackTypes.map((typeOption) => {
                  const IconComponent = typeOption.icon;
                  const isSelected = feedback.type === typeOption.type;
                  
                  return (
                    <button
                      key={typeOption.type}
                      type="button"
                      onClick={() => handleTypeSelect(typeOption.type)}
                      className={`p-3 rounded-lg border-2 transition-all duration-200 flex flex-col items-center space-y-1 ${
                        isSelected
                          ? `${typeOption.bgColor} ${typeOption.borderColor} ${typeOption.color}`
                          : 'bg-gray-50 border-gray-300 text-gray-600 hover:bg-gray-100'
                      }`}
                    >
                      <IconComponent className="text-2xl" />
                      <span className="text-xs font-medium">{typeOption.label}</span>
                    </button>
                  );
                })}
              </div>
            </div>

            {/* Comment */}
            <div className="mb-4">
              <label className="block text-sm font-medium text-gray-700 mb-2">
                Your Comments *
              </label>
              <textarea
                value={feedback.comment}
                onChange={(e) => setFeedback(prev => ({ ...prev, comment: e.target.value }))}
                placeholder="Share your detailed experience about this event..."
                className="w-full border border-gray-300 px-3 py-2 rounded-lg focus:outline-none focus:ring-2 focus:ring-[#555879] focus:border-transparent"
                rows={4}
                maxLength={500}
                required
              />
              <div className="text-xs text-gray-500 mt-1">
                {feedback.comment.length}/500 characters
              </div>
            </div>

            {/* Anonymous Option */}
            <div className="mb-6">
              <label className="flex items-center space-x-2">
                <input
                  type="checkbox"
                  checked={feedback.isAnonymous}
                  onChange={(e) => setFeedback(prev => ({ ...prev, isAnonymous: e.target.checked }))}
                  className="rounded border-gray-300 text-[#555879] focus:ring-[#555879]"
                />
                <span className="text-sm text-gray-700">Submit feedback anonymously</span>
              </label>
            </div>

            {/* Submit Buttons */}
            <div className="flex space-x-3">
              <button
                type="button"
                onClick={onClose}
                className="flex-1 px-4 py-2 border border-gray-300 text-gray-700 rounded-lg hover:bg-gray-50 transition-colors"
                disabled={isSubmitting}
              >
                Cancel
              </button>
              <button
                type="submit"
                disabled={isSubmitting || !feedback.comment.trim() || !feedback.type || feedback.rating === 0}
                className="flex-1 px-4 py-2 bg-[#555879] text-white rounded-lg hover:bg-[#4A4C6A] transition-colors disabled:bg-gray-400 disabled:cursor-not-allowed"
              >
                {isSubmitting ? 'Submitting...' : (existingFeedback ? 'Update Feedback' : 'Submit Feedback')}
              </button>
            </div>
          </form>
        </div>
      </div>
    </div>
  );
}